...
def Test(save_path: "Pathlib.Path",
    ...
    print("test_f1 = {:10.8f}  time = {:4.2f}".format(test_f1score, time.time() - start_time))
    print(f"accuracy_score: {round(float(accuracy_score(y_true, y_pred)) * 100, 2)}")
    print(f"precision_score: {round(float(precision_score(y_true, y_pred, average='micro', zero_division=0)) * 100, 2)}")
    print(f"recall_score: {round(float(recall_score(y_true, y_pred, average='micro', zero_division=0)) * 100, 2)}")
